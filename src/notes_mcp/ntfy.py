...
NOTIFICATION_RULES: dict[str, NotifyRule] = {
    # Startup — confirm notifications are working
    "server.ready": NotifyRule(
        priority="default", debounce_s=0, tags="white_check_mark"
    ),
    # Vault missing on startup — urgent
    "server.vault_not_found": NotifyRule(
        priority="urgent", debounce_s=0, tags="rotating_light"
    ),
}

# Keys to extract from event_dict for notification body
_CONTEXT_KEYS = ("error", "stderr", "message", "path", "detail")
...
class NtfyNotifier:
    ...
    def _send_sync(
        self, title: str, message: str, priority: str, tags: str
    ) -> None:
